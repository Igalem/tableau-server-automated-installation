...
# ----------------------------------------- Setting parameters:
HOSTNAME=$(hostname -i)
INIT_HOST=#Set the initial node to be attach to#
...
sudo firewall-cmd --permanent --add-rich-rule="rule family=ipv4 source address=$INIT_HOST/32 port port=8000-9000 protocol=tcp accept"
sudo firewall-cmd --permanent --add-rich-rule="rule family=ipv4 source address=$INIT_HOST/32 port port=27000-27010 protocol=tcp accept"
